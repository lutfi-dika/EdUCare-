...
@section('title', 'EduCare - Pesan Saya')
...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-[#111827] dark:text-[#ededed]" data-i18n="s_msg.title">Pesan & Diskusi</h1>
    <p class="text-[#64748B] dark:text-[#a3a3a3] mt-1" data-i18n="s_msg.sub">Komunikasi dengan guru.</p>
</div>
        ...
        <div class="bg-white rounded-2xl border border-[#E2E8F0] dark:bg-[#1a1a1a] dark:border-[#2a2a2a] overflow-hidden">
            <div class="p-4 border-b border-[#E2E8F0] dark:border-[#2a2a2a]">
                <button onclick="document.getElementById('newMessageModal').classList.remove('hidden')" class="w-full px-4 py-2.5 bg-[#2563EB] text-white text-sm font-medium rounded-xl hover:bg-[#1D4ED8] transition-colors">
                    <span data-i18n="s_msg.pesan_baru">+ Pesan Baru</span>
                </button>
            </div>
            <div class="divide-y divide-[#E2E8F0] dark:divide-[#2a2a2a] max-h-[500px] overflow-y-auto">
                @forelse($conversations as $conv)
                    <a href="{{ url('/student/messages/' . $conv['user']->id) }}" class="flex items-center gap-3 p-4 hover:bg-[#F8FAFC] dark:hover:bg-[#0f172a] transition-colors">
                ...
                @empty
                    <div class="p-6 text-center">
                        <p class="text-sm text-[#64748B] dark:text-[#a3a3a3]" data-i18n="s_msg.belum_ada">Belum ada percakapan.</p>
                    </div>
                @endforelse
            </div>
        </div>
            ...
            @else
                <div class="flex-1 flex items-center justify-center">
                    <div class="text-center">
                        ...
                        <p class="text-[#64748B] dark:text-[#a3a3a3]" data-i18n="s_msg.pilih_chat">Pilih percakapan atau mulai pesan baru</p>
                    </div>
                </div>
            @endif
        </div>
...
<div id="newMessageModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-[#1a1a1a] rounded-2xl w-full max-w-md p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="font-bold text-[#111827] dark:text-[#ededed]" data-i18n="s_msg.pesan_baru">Pesan Baru</h3>
        ...
        <form action="{{ url('/student/messages/send') }}" method="POST">
            @csrf
            <div class="mb-4">
                <label class="block text-sm font-medium text-[#64748B] dark:text-[#a3a3a3] mb-1" data-i18n="s_msg.kepada">Kepada</label>
                <select name="receiver_id" class="w-full px-4 py-2.5 bg-[#F8FAFC] dark:bg-[#2a2a2a] border border-[#E2E8F0] dark:border-[#475569] rounded-xl text-sm text-[#111827] dark:text-[#ededed]" required>
                    <option value="" data-i18n="s_msg.pilih_guru">Pilih guru...</option>
            ...
            <div class="mb-4">
                <label class="block text-sm font-medium text-[#64748B] dark:text-[#a3a3a3] mb-1"><span data-i18n="s_msg.terkait">Terkait Materi</span> <span data-i18n="s_msg.opsional">(opsional)</span></label>
                <select name="material_id" class="w-full px-4 py-2.5 bg-[#F8FAFC] dark:bg-[#2a2a2a] border border-[#E2E8F0] dark:border-[#475569] rounded-xl text-sm text-[#111827] dark:text-[#ededed]">
                    <option value="" data-i18n="s_msg.tanpa_materi">Tanpa materi</option>
            ...
            <div class="mb-4">
                <label class="block text-sm font-medium text-[#64748B] dark:text-[#a3a3a3] mb-1" data-i18n="s_msg.pesan">Pesan</label>
            ...
            <button type="submit" class="w-full px-4 py-2.5 bg-[#2563EB] text-white text-sm font-medium rounded-xl hover:bg-[#1D4ED8] transition-colors" data-i18n="s_msg.kirim">Kirim Pesan</button>
        </form>
    </div>
</div>
